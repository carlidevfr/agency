...
class Type extends Model
{
    ...
    public function getRelatedType($typeId)
    // Récupère tous les éléments liés à un type
    {
        try {
            ...
            foreach ($tables as $tableName => $foreignKey) {
                ...
                $bdd = $this->connexionPDO();
                $req = "SELECT * FROM $tableName WHERE $foreignKey = :typeId";

                // on teste si la connexion pdo a réussi
                if (is_object($bdd)) {
                    $stmt = $bdd->prepare($req);

                    if (!empty ($typeId) and !empty ($typeId)) {
                        $stmt->bindValue(':typeId', $typeId, PDO::PARAM_INT);
                        if ($stmt->execute()) {
